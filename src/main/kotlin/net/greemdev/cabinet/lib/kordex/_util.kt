...
fun<T : TextInputWidget<T>> T.get(): String {
    return if (required)
        value!!
    else value ?: ""
}

fun<T : TextInputWidget<T>> T.getListValue(separator: Char = ';'): List<String> = when (val str = get()) {
    "" -> listOf()
...
inline fun<reified T : Any> dynamicKoinComponent() = invoking<T>(::koinComponent)